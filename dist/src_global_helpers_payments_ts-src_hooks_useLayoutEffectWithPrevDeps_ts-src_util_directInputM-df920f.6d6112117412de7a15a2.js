...
function buildStarsTransactionCustomPeer(peer, isForTon) {
...
function formatStarsTransactionAmount(lang, currencyAmount) {
  ...
  if (currencyAmount.currency === _config__WEBPACK_IMPORTED_MODULE_0__.TON_CURRENCY_CODE) {
    const amount = (0,_util_formatCurrency__WEBPACK_IMPORTED_MODULE_2__.convertCurrencyFromBaseUnit)(currencyAmount.amount, currencyAmount.currency);
    const absAmount = Math.abs(amount);
    const tonText = lang('TonAmountText', {
      amount: absAmount
    }, {
      pluralValue: absAmount
    });
    if (amount < 0) {
      return `- ${tonText}`;
    }
...
//# sourceMappingURL=src_global_helpers_payments_ts-src_hooks_useLayoutEffectWithPrevDeps_ts-src_util_directInputM-df920f.6d6112117412de7a15a2.js.map