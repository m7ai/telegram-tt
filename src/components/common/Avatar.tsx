...
import type { ApiPeer, ApiPhoto, ApiWebDocument } from "../../api/types";
...
import buildClassName, {
  createClassNameBuilder,
} from "../../util/buildClassName";
import buildStyle from "../../util/buildStyle";
import { isUserId } from "../../util/entities/ids";
import { getFirstLetters, formatIntegerCompact } from "../../util/textFormat";
...
const Avatar: FC<OwnProps> = ({
  ...
  const isRoundedRect =
    (isCustomPeer && peer.isAvatarSquare) ||
    ...
    >
      <div className="inner">
        {typeof content === "string"
          ? renderText(content, [isBig ? "hq_emoji" : "emoji"])
          : content}
      </div>
      {Boolean(unreadCount && unreadCount > 0 && !isSavedDialog) && (
        <div className="avatar-unread">
          {formatIntegerCompact(newLang, unreadCount!)}
        </div>
      )}
